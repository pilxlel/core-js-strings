...
function removeFirstOccurrences(str, value) {
  const valuePos = str.indexOf(value);
  const valueLength = value.length;
  let result = '';

  if (valuePos === -1) return str;

  result = str.slice(0, valuePos);
  result += str.slice(valuePos + valueLength);
  return result;
}
...
function removeLastOccurrences(str, value) {
  const valuePos = str.lastIndexOf(value);
  const valueLength = value.length;
  let result = '';
...
function isPalindrome(str) {
  ...
  const modifiedStr = str.replace(regExp, '').toLowerCase();
  const modifiedStrReverted = modifiedStr.split('').reverse().join('');
  if (modifiedStr === modifiedStrReverted) return true;
  return false;
}
...
function extractNameFromTemplate(value) {
  const regExpLeft = /(Hello, )/g;
  const regExpRight = /(!)/g;

  let res = value.replace(regExpLeft, '');
  res = res.replace(regExpRight, '');
  return res;
}
